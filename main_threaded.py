...
def main():
	...
	fast_thread = threading.Thread(target=fast_loop, args=(print_lock, state_vector_lock, imu, enc, iss, oss, rss), name="fast_loop")
	fast_thread.start()
	slow_thread = threading.Thread(target=slow_loop, args=(print_lock, path, ctrl), name="slow_loop")
	slow_thread.start()
...
def slow_loop(print_lock, path, ctrl):
	p = path.pop(0)
	with print_lock:
		print(p)
	while True:
